...
const CONDITIONS = [
  ...
  { number: "02", title: "교리", desc: "공개 미군 교리 · MIL-STD-2525" },
  { number: "03", title: "온톨로지", desc: "Palantir Foundry 기반 (16개 전장 객체 타입)" },
  { number: "04", title: "C4I 체계", desc: "지휘결심 지원 플랫폼 (Palantir Foundry + AIP)" },
];
      ...
      <div className="flex h-full w-full flex-col px-[140px] pt-[140px] pb-[120px]">
        ...
        <div className="mt-10 flex flex-col gap-6 flex-1 justify-center">
          {CONDITIONS.map((c, i) => (
            ...
            >
              <span className="text-[48px] font-extrabold text-accent leading-none w-[90px] shrink-0">
                {c.number}
              </span>
              <span className="text-[30px] font-bold text-fg">
                {c.title}
              </span>
              <span className="text-[26px] text-fg-muted">
                — {c.desc}
              </span>
            </motion.div>
          ))}
        ...
        >
          ...
          <p className="mt-2 text-[34px] font-bold text-accent">
